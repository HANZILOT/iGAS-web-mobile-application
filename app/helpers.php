...
{
     /**
     * check if user has already rated the gasoline station
     */
    function isRatedByAuthUser($gasoline_station) 
    {
        $check_record = \App\Models\Rating::where('user_id', auth()->id())
        ->where('gasoline_station_id', $gasoline_station)
        ->whereMonth('created_at', now())
        ->first();

        return $check_record ? true : false; // check if the user has already rated the gasoline station
    }
}
